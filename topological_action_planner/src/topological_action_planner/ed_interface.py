...
def pose_stamped(x, y, z=0.0, qx=0.0, qy=0.0, qz=0.0, qw=1.0, frame_id="map"):
    return PoseStamped(
        ...
        pose=Pose(position=Point(x, y, z), orientation=Quaternion(qx, qy, qz, qw)),
    )
...
class EdInterface:
    def __init__(self, robot_name, tf_buffer):
        ...
        self._get_constraint_srv = rospy.ServiceProxy(
            "/%s/ed/navigation/get_constraint" % robot_name, GetGoalConstraint
        )
    ...
    def get_room(self, entity: str, area: str) -> str:
        ...
        mapping = {
            "coffee_table": {
                "in_front_of": "test_area2",
                "in_front_of2": "breakfast_room",
            },
            "biestheuvel_door": {"in_front_of": "test_area"},
            "door": {"in_front_of": "test_area", "in_front_of2": "test_area2"},
            "robot": {"": "test_area"},
        }
